...
#include "module_genai/modules/m_llm_inference_pa/models/deepseek_r1_distill_qwen_1_5b.hpp"
...
const ModuleSpec& LLMInferencePAModule::get_spec() {
    static const ModuleSpec spec = []() {
        ...
        s.add_param(
            ...
            "false",
            true,
            "Whether to split text model for better performance, only effective when text_model_path is provided");
        return s;
    }();
    return spec;
}
...
LLMInferencePAModule::PTR LLMInferencePAModule::create(const IBaseModuleDesc::PTR& desc,
    ...
    if (desc->model_type == "deepseek_r1_distill_qwen_1_5b" ||
        desc->model_type == "deepseek_r1_distill_qwen_1.5b") {
        return std::make_shared<ov::genai::module::LLMInferencePAModule_DeepSeekR1DistillQwen1_5B>(
            desc,
            pipeline_desc,
            VLMModelType::QWEN3_5);
    }
...
LLMInferencePAModule::InputsParams::PTR LLMInferencePAModule::parse_inputs(InputsParams::PTR inputs_params) {
    ...
    OPENVINO_ASSERT(
        !(inputs_params->has_input_ids && (inputs_params->has_input_embeds || inputs_params->has_merged_embeds)),
