...
void init_module_pipeline(py::module_& m) {
    py::class_<ov::genai::module::ModulePipeline>(m,
        ...
        .def(py::init([](const std::filesystem::path& yaml_cfg, const py::kwargs& kwargs) {
                 return std::make_unique<ov::genai::module::ModulePipeline>(yaml_cfg);
             }),
             py::arg("yaml_cfg"),
             "config file path",
             R"(
            Module Pipeline class constructor.
            yaml_cfg (os.PathLike): Path to the configuration YAML file.
        )")
