...
inline ov::AnyMap parse_inputs_from_yaml_cfg_for_video_generation(const std::filesystem::path& cfg_yaml_path,
    ...
    for (const auto& entry : input_params) {
        ...
        if (param_type == "String" && utils::contain_key(param_name, {"negative_prompt"})) {
        ...
        if (param_type == "Int" && utils::contain_key(param_name, {"num_frames"})) {
            if (num_frames.empty()) {
        ...
        if (param_type == "Int" && utils::contain_key(param_name, {"batch_size"})) {
            if (batch_size.empty()) {
        ...
        if (param_type == "Int" && utils::contain_key(param_name, {"num_images_per_prompt", "num_videos_per_prompt"})) {
        ...
        if (param_type == "Int" && utils::contain_key(param_name, {"seed"})) {
            if (seed.empty()) {
